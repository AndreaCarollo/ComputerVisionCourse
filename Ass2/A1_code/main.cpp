...
// this function is used to read the ground_truth and generate the vector of exact points
void ss(const char *ground_truth, vector<Rect> *exact_points, int i);

// this function calculate the center of a cv::Rectangle
...
int main()
{
    ...
    // To store frames in video
    VideoWriter VideoD("Output/Videoout/Video_Detection.avi", VideoWriter::fourcc('M', 'J', 'P', 'G'), 5, Size(768, 576), true);

    VideoWriter VideoT("Output/Videoout/Video_Tracking.avi", VideoWriter::fourcc('M', 'J', 'P', 'G'), 5, Size(768, 576), true);
    if (!VideoD.isOpened() || !VideoT.isOpened())
    {
        cout << "codec failed\n";
        return 0;
    }

    // enable possibility to resize the window of the output
    ...
    for (int i = 0; i < 1000; i++)
    {
        ...
        if ((i % refresh_tracker == 0 || flag_refresh == true) && !(pedestrian.empty()))
        {
            ...
            for (int k = 0; k < new_ROIs.size(); k++)
            {
                ...
                for (int l = k + 1; l < new_ROIs.size(); l++)
                {
                    ...
                    // r5 smallest rectangle that include r1 and r2
                    r5 = (r1 | r2).area();
        ...
        // compute cost matrix of distances of new and old last point of each trajectories
        vector<vector<double>> costMatrix_traj;
        if (i != 0 && !(trajectories.empty()) && !(ROIs.empty()))
        {
            // creating a list of point = center of the rectangles
            for (int j = 0; j < ROIs.size(); j++)
            {
            ...
            for (int j = 0; j < assignment_traj.size(); j++)
            {
                ...
                if (tmp_index != -1 && costMatrix_traj[j][tmp_index] < 40) // it's an old trace
                {
                ...
                else if (tmp_index != -1) // it's a new trace to store
                {
                    last_ID++;
        ...
        else // initialize the trajectories first points
        {
            for (int j = 0; j < ROIs.size(); j++)
            {
                new_center_list.push_back(center_rect(ROIs[j]));
        ...
        for (int k = 0; k < trajectories.size(); k++)
        {
            if (trajectories[k].size() > 1)
            {
                for (int j = 1; j < trajectories[k].size(); j++)
                {
                    ...
                    line(frame, pt_1, pt_2, Scalar(0,255,0), 2, 8, 0);
                }
            }
        }
        ...
        {
            int id = IDs[k];
            int x_center = trajectories[k].back().x;
            int y_center = trajectories[k].back().y;
        ...
        for (int j = 0; j < trackers.getObjects().size(); j++)
        {
            ...
            //         FONT_HERSHEY_TRIPLEX, 0.4, Scalar(255, 0, 0), 1, 8);
        }

        for (int j = 0; j < IDs.size(); j++)
        {
            putText(frame, to_string(IDs[j]),
                    trajectories[j].back(),
                    FONT_HERSHEY_TRIPLEX, 0.5, Scalar(0, 0, 255), 1, 8);
        }
        ...
        char nomefile_w[30];
        std::sprintf(nomefile_w, "./Output/PedTrack/%06d.jpg", i);
        cv::imwrite(nomefile_w, frame);

        // write frames on videos
        VideoD.write(frame_det);
        VideoT.write(frame);
