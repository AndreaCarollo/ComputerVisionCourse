...
int main()
{
    ...
    std::vector<Ptr<Tracker>> new_algorithms, old_algorithms;
    ...
    // add object to track
    // mtkr_alt.addTarget(frame, ROIs[i], TrackerCSRT::create());
    ...
    for (int i = 0; i < 1000; i++)
    {
        ...
        vector<Rect> new_ROIs, old_ROIs;
        vector<Rect2d> new_objects, old_objects;
        ...
        int ppp = -1;
        // find new pedestrian
        for (int k = 0; k < pippo; pippo++)
        {
            old_ROIs.push_back(ROIs[k]);
            old_algorithms.push_back(TrackerCSRT::create());
        }
        //trackers.add(algorithms, frame, objects);
        new_algorithms.clear();
        new_ROIs = old_ROIs;
        new_algorithms = old_algorithms;
        bool add_flag = false;
        bool change_tkr = false;
        for (int k = 0; k < pedestrian.size(); k++)
        {
            ...
            {
                ROIs = new_ROIs;
                new_objects.push_back(ROIs[k]);
                new_algorithms.push_back(TrackerCSRT::create());
            }
        }
        cout << i << "-" << ppp << endl;
        ...
        if (change_tkr = true)
        {
            ...
            newtrackers.add(algorithms, frame, new_objects);
        ...
        cv::imshow("tracker + detector", test);
