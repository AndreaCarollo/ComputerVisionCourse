...
int main()
{
    ...
    for (int i = 0; i < 1000; i++)
    {
        ...
        const char *stringa_test = "../Video/gt/gt.txt";
        ...
        // vector<Point> tmp_mat;
        vector<vector<Rect>> old_trajectories = trajectories;

        vector<vector<Rect>> new_trajectories;
        vector<Rect> new_rectangles;
        vector<Rect> old_rectangles;
        // ROIs;

        // if (!(old_trajectories.empty()))
        // {
        //     for (int k = 0; k < old_trajectories.size(); k++)
        //     {
        //         old_rectangles.push_back(old_trajectories[k].back());
        //     }

        //     for (int k = 0; k < old_rectangles.size(); k++)
        //     {
        //         float tmp_smallest_distance = 25;
        //         for (int j = 0; j < ROIs.size(); j++)
        //         {
        //             // compute distance
        //             Point tmp_old = (old_rectangles[k].tl() + old_rectangles[k].br()) / 2.0;
        //             Point tmp_new = (new_rectangles[k].tl() + new_rectangles[k].br()) / 2.0;
        //             float tmp_distance = sqrt((tmp_old.x - tmp_new.x) ^ 2 + (tmp_old.y - tmp_new.y) ^ 2);
        //             cout << tmp_distance << endl;
        //             if (tmp_distance < tmp_smallest_distance)
        //             {
        //                 // tmp_smallest_distance = tmp_distance;
        //                 old_trajectories[k].push_back(ROIs[j]);
        //                 new_trajectories.push_back(old_trajectories[k]);
        //                 break;
        //             }
        //             // else
        //             // {
        ...
        //     //     vector<Rect> tmp_vec;
        //     //     tmp_vec.push_back(ROIs[k]);
        //     //     trajectories.push_back(tmp_vec);
        //     // }
        ...
        // Plot line of trajectories
        for (int k = 0; k < trajectories.size(); k++)
        {
            for (int j = 1; j < trajectories[k].size(); j++)
            {
                Point pt_1 = (trajectories[k][j - 1].tl() + trajectories[k][j - 1].br()) / 2;
                Point pt_2 = (trajectories[k][j].tl() + trajectories[k][j].br()) / 2;
                line(frame, pt_1, pt_2, Scalar(0, 255, 0), 1, 8, 0);
            }
        }
